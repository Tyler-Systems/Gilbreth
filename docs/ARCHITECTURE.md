...
- **Windows shutdown/logoff boundaries.** The same hidden top-level system window answers `WM_QUERYENDSESSION` with "allow" and handles `WM_ENDSESSION` by posting `WM_QUIT` to the capture pump. That routes OS-initiated shutdown/logoff through the same post-loop flush path as tray Quit: pending mouse movement is flushed, the current foreground dwell is closed, live windows emit synthesized closes, the final sender drops, and the writer drains/stamps the session before process exit as far as Windows allows.
