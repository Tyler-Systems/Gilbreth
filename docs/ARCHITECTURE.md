...
- **Windows shutdown/logoff boundaries.** The same hidden top-level system window answers `WM_QUERYENDSESSION` with "allow" and handles `WM_ENDSESSION` by posting `WM_QUIT` to the capture pump. `WM_CLOSE` routes to the same quit path (added 2026-07-28): every realistic sender — `taskkill` without `/F`, Task Manager's polite path, installer close requests — means "exit gracefully," and the unhandled default destroyed just the window, leaving a ghost tray over a still-running recorder; a stoppable posture beats resisting polite termination. That routes OS-initiated shutdown/logoff through the same post-loop flush path as tray Quit: pending mouse movement is flushed, the current foreground dwell is closed, live windows emit synthesized closes, the final sender drops, and the writer drains/stamps the session before process exit as far as Windows allows.
