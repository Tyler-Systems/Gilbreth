...
| Linux | LIN-2: ambient X11 capture at the parity remainder (the dogfood tier) and the dashboard build and run from source, with the StatusNotifier tray, XGrabKey pause hotkey, and XDG autostart. Session lock/console and power boundaries ride one D-Bus watcher thread (elogind composed with the session locker's surface), window lifecycle rides the client list, and clipboard metadata rides an XFixes-driven TARGETS round trip. Modal dialogs are the product's own egui shell in a child process, so confirm-gated privacy actions work; the key-content opt-in is absent rather than unprotected (no X11 password-field probe in this tier, and raw input is discarded while the session is locked). Wayland is absent by design; there is no Linux application release lane. |
