...
/// Power-boundary state machine, hoisted 2026-08-01 when the Linux port
/// became the macOS machine's byte-for-byte twin (the
/// ForegroundState/ProcessMonitor precedent; bodies moved unchanged, unit
/// tests moved with them, two platform-named log strings neutralized).
/// The macOS pump feeds it `IORegisterForSystemPower` edges with
/// `mach_continuous_time` as the spans-sleep clock; the Linux pump feeds
/// it elogind `PrepareForSleep` edges with `CLOCK_BOOTTIME`. Both
/// platforms' `Instant` pauses during sleep, so spans-sleep minus instant
/// across one pump pass IS the slept interval the notification path
/// missed — the divergence detector that replaces the Windows tick-gap
/// detector. The Windows backend keeps its own message-driven handling.
pub mod power {
    use std::time::{Duration, Instant};

    use tracing::{debug, info};

    use crate::{Captured, EventPayload, Source};

    /// Ported Windows constant: a resume within this window of the previous
    /// one, with no suspend between, is the same wake reported twice (a
    /// defensive guard here).
    const POWER_RESUME_DEBOUNCE: Duration = Duration::from_secs(2);
    /// Ported Windows threshold: a slept interval this large with no
    /// observed boundary is a missed boundary worth a
    /// `PowerBoundaryRecovered` row.
    const MISSED_POWER_BOUNDARY_THRESHOLD_MS: u64 = 30_000;
    /// Status snapshots poll on the Windows 1 s system cadence (the
    /// recorded poll-with-edge-detection rule); rows emit on change only.
    const STATUS_SAMPLE_INTERVAL: Duration = Duration::from_secs(1);

    /// A sleep/wake notification queued by the platform's edge source.
    /// `continuous_ms` is read at receipt (the closest observable point to
    /// the edge on the spans-sleep clock); `at` is the `Instant` stamped
    /// alongside it.
    #[derive(Clone, Copy, Debug)]
    pub struct PowerEdgeSample {
        pub at: Instant,
        pub continuous_ms: Option<u64>,
        pub edge: PowerEdge,
    }

    #[derive(Clone, Copy, Debug, Eq, PartialEq)]
    pub enum PowerEdge {
        /// Suspend committed (`kIOMessageSystemWillSleep` /
        /// `PrepareForSleep(true)`): Gilbreth observes sleep, never delays
        /// it.
        WillSleep,
        /// Wake complete (`kIOMessageSystemHasPoweredOn` /
        /// `PrepareForSleep(false)`). Every wake emits, dark wakes
        /// included (owner decision: Windows `PBT_APMRESUMEAUTOMATIC`
        /// parity; user presence stays the job of idle/active and dwell,
        /// not power rows).
        DidWake,
    }

    /// AC/battery snapshot. `None` fields are honestly unknown — a machine
    /// without a battery has no percentage to report, mirroring the
    /// Windows desktop's unknown battery fields. `battery_saver` carries
    /// macOS Low Power Mode and stays `None` on Linux (no analog in that
    /// tier).
    #[derive(Clone, Copy, Debug, Eq, PartialEq)]
    ...
    /// snapshot. Production is the IOKit source on macOS and the D-Bus
    /// watcher queue beside the boottime/sysfs reads on Linux; tests
    /// inject scripts.
    pub trait PowerSource {
        /// Everything the platform's sleep/wake listener queued since the
        /// last pass.
        fn drain_edges(&mut self) -> Vec<PowerEdgeSample>;
        /// The spans-sleep clock in ms. `None` disables the divergence
        /// detector for the pass (quiet test sources).
    ...
    /// What the pump must do for a boundary this pass, in Windows order:
    /// input state machines reset and the Foreground segment closes BEFORE
    /// the power rows are pushed (`on_power_suspend_at` ordering, ported).
    pub struct PowerBoundary {
        /// Close the open Foreground segment before pushing `rows` (false
        /// when a matched resume follows a suspend that already closed
        /// it).
        pub close_foreground: bool,
        pub rows: Vec<Captured>,
    }

    pub struct PowerMonitor {
        suspended: bool,
        last_resume_at: Option<Instant>,
        /// (instant, spans-sleep ms) of the last divergence sample; the
        /// two clocks tick together while awake and diverge by exactly
        /// the slept time across a sleep.
        divergence_sample: Option<(Instant, u64)>,
        /// Windows' `last_power_status` key: (ac, saver, percent/10) —
        /// only meaningful changes emit (plug/unplug, saver toggle, ~10%
        /// steps).
        status_key: Option<(Option<bool>, Option<bool>, Option<u8>)>,
        /// The first status sample baselines silently: Windows only
        /// samples on change messages, so its rows are actual changes — a
        /// poller that emitted its first sample would add a spurious row
        /// per launch (the recorded macOS amendment, kept).
    ...
    impl PowerMonitor {
        ...
        /// The suspend edge: the Windows `PBT_APMSUSPEND` twin.
        pub fn on_will_sleep(&mut self, sample: &PowerEdgeSample) -> Option<PowerBoundary> {
            if self.suspended {
                debug!("duplicate suspend edge ignored (already suspended)");
        ...
        /// The wake edge: the Windows `PBT_APMRESUME*` twin, including the
        /// ported duplicate-resume debounce and the missed-boundary
        /// recovery for a wake whose suspend was never seen.
        pub fn on_did_wake(&mut self, sample: &PowerEdgeSample) -> Option<PowerBoundary> {
            if !self.suspended
            ...
            if !matched_suspend {
                // The suspend was never observed. If the clocks diverged
                // past the threshold, this wake also recovers a missed
                // boundary — Windows' resume-path recovery, rows in the
                // same order (Recovered, then Resume).
            ...
            Some(PowerBoundary {
                // A matched resume follows a suspend that already closed
                // the segment; an unmatched one must close it now (capped
                // by the foreground monitor's own gap-capped close — on
                // both consumers sleep contributes no dwell anyway, the
                // dwell clock pauses).
        ...
        /// Per-pass divergence detector — the replacement for the Windows
        /// tick-gap detector: `Instant` pauses during sleep on both
        /// consumers, the spans-sleep clock does not, so spans-sleep-delta
        /// minus instant-delta across one pass is the slept time the
        /// notification path missed.
        pub fn poll_divergence(
            &mut self,
            now: Instant,
            ...
            if self.suspended {
                ...
                // arrived: the divergence is the wake evidence. Matched
                // resume; the segment was already closed at suspend.
                self.suspended = false;
            ...
            // Neither boundary was observed: the Windows timer-path
            // recovery — a Recovered row only, no Resume row (ported
            // semantics).
            self.last_resume_at = Some(now);
        ...
        /// Throttled status sample with the ported Windows change key.
        /// `force` bypasses the throttle right after a boundary (Windows
        /// samples status at every real resume and recovery); the silent
        /// first baseline still applies — a boundary at launch must not
        /// fabricate a change row.
        pub fn poll_status<S>(
            &mut self,
            now: Instant,
            force: bool,
            snapshot: &mut S,
        ...
        fn divergence_ms(&mut self, now: Instant, continuous_ms: Option<u64>) -> Option<u64> {
            ...
            let instant_delta = u64::try_from(now.saturating_duration_since(last_at).as_millis())
                .unwrap_or(u64::MAX);
    ...
    fn recovered_row(at: Instant, gap_ms: u64) -> Captured {
        Captured::new(
            ...
            EventPayload::PowerBoundaryRecovered {
                gap_ms,
                // Windows reports the 30 s cap it enforces on dwell
                // attributed across the gap (its tick clock spans
                // suspend). On both consumers the dwell clock pauses
                // during sleep, so a gap can contribute NO dwell — 0 is
                // the cap actually in force (the recorded macOS
                // amendment; the field means "max dwell attributable
    ...
    mod tests {
        ...
        fn will_sleep_emits_suspend_and_closes_foreground() {
            ...
            let boundary = monitor
                ...
                .expect("first suspend edge is a boundary");
        ...
        fn unmatched_wake_with_divergence_recovers_the_missed_boundary() {
            let mut monitor = PowerMonitor::new();
            let t0 = Instant::now();
            // Baseline the divergence clocks, then wake with the
            // spans-sleep clock 60 s ahead of the instant clock: the
            // suspend was missed.
            ...
            let EventPayload::PowerBoundaryRecovered { gap_ms, .. } = boundary.rows[0].payload
            else {
        ...
        fn divergence_while_suspended_is_the_recovered_matched_resume() {
            ...
            // spans-sleep clock 45 s ahead of the instant clock.
            let boundary = monitor
        ...
        fn status_baselines_silently_then_emits_meaningful_changes_only() {
            ...
            // A saver flip is a change even inside the throttle window
