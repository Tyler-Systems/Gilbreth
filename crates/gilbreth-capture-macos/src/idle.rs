//! Idle stream (the macOS TCC and stream rules: no TCC
//! permission). The edge detector is ported verbatim from
//! `gilbreth-capture-windows`'s `IdleState` so both platforms cross the
//! threshold identically; the sample source is the system HID idle clock
//! read through CoreGraphics (see `coregraphics.rs` and the amended TCC
//! record — same value the recorded IOKit property exposes, without a new
//! vendor lineage).
//!
//! Sampling cadence matches the Windows system timer (1s,
//! `SYSTEM_POLL_INTERVAL_MS`) rather than the pump's 50ms service tick;
//! `IdleMonitor` throttles internally. Windows also has an immediate
//! `Active` edge on raw input delivery — a path that only exists when its
//! input streams are enabled, so in this tier's comparable configuration
//! both platforms detect activity at timer granularity (≤1s here); the
//! immediate path arrives with the input-tap slice.
//!
//! On a mid-run stream disable the state resets to fresh, so re-enabling
//! never emits an `Active` row for an idle period that began while the
//! stream was off. (This borrows the Windows *reseed* mechanism; a bare
//! Windows stream toggle does not reseed and can emit exactly that orphan
//! `Active` row — the mac behavior is deliberately the cleaner of the
//! two, reviewed in the Idle/System round.)

use std::time::{Duration, Instant};

use gilbreth_core::{Captured, EventPayload, Source};

/// Windows `SYSTEM_POLL_INTERVAL_MS`: the finer of its two timer cadences
/// (1s when the System stream is on, 5s idle-only). One cadence keeps the
/// mac pump simple and samples at least as often as Windows ever does.
pub(crate) const SAMPLE_INTERVAL: Duration = Duration::from_secs(1);

/// Ported from `gilbreth-capture-windows::IdleState`: same threshold edge
/// rules, same payloads, same `Source::System` labeling (the payloads
/// gate under `CaptureStream::Idle` via `stream_for`).
struct IdleState {
    threshold: Duration,
    is_idle: bool,
}
...
/// Samples the HID idle clock on the Windows cadence and feeds the edge
    ...
    P: FnMut() -> Option<u64>,
{
    ...
    pub(crate) fn poll(&mut self, now: Instant, enabled: bool, events: &mut Vec<Captured>) {
        if !enabled {
            if self.enabled {
                // Fresh state on disable, exactly like the Windows reseed:
                // an idle period that begins while the stream is off never
                // yields a later Active edge.
...
mod tests {
    ...
    fn idle_and_active_edges_fire_once_per_crossing() {
        ...
        assert!(matches!(
            events[0].payload,
            EventPayload::Idle { idle_ms: 60_000 }
        ));
        ...
        assert!(matches!(
            events[1].payload,
            EventPayload::Active { idle_ms: 200 }
        ));
    }
    ...
    fn sampling_is_throttled_to_the_windows_cadence() {
        ...
        // 50ms later the pump polls again; the monitor must not resample —
        // an Active edge (idle reset) within the same second is deferred to
        // the next 1s sample, same as Windows' timer granularity.
        idle.set(Some(10));
    ...
    fn disable_resets_state_so_reenable_never_reports_off_period_idleness() {
        ...
        // While off, the user returned (idle reset). Re-enable: fresh state
        // sees a below-threshold sample with no is_idle flag carried over,
        // so no Active edge fabricates a transition that happened off-book.
        idle.set(Some(100));
        monitor.poll(base + Duration::from_secs(10), true, &mut events);
        assert_eq!(events.len(), 1, "no Active edge from the off period");

        // The detector still works after the reset.
        idle.set(Some(2_000));
